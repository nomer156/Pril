// Firebase через CDN (совместимо с GitHub Pages + ES-модули)
...
// Авторизация не обязательна, но если включена в консоли — будет норм
try { signInAnonymously(getAuth(app)).catch(()=>{}); } catch {}

// ОДНА ОБЩАЯ «КОМНАТА» ДЛЯ ДВОИХ
const ROOT = ref(db, "rooms/default");
...
// Первичная инициализация данных (ленивая)
ensure("points", { you: 0, her: 0 });
ensure("names",  { you: "Ты", her: "Она" });

// Подписки
onValue(child("points"), (s) => {
  const v = s.val() || { you:0, her:0 };
  scoreYou.textContent = v.you ?? 0;
  scoreHer.textContent = v.her ?? 0;
});

onValue(child("names"), (s) => {
...
onValue(child("goals"), (s) => renderGoals(s.val() || {}));
onValue(child("history"), (s) => renderHistory(s.val() || {}));
...
// Helpers
function child(path){ return ref(db, `rooms/default/${path}`); }
function ensure(path, defaults){ update(child(path), defaults).catch(()=>{}); }

function addPoints(who, delta, reason=''){
  onValue(child("points"), (snap)=>{
    const cur = snap.val() || { you:0, her:0 };
    const next = { ...cur, [who]: (parseInt(cur[who]||0,10) + delta) };
    update(child("points"), next);
  }, { onlyOnce: true });

  push(child("history"), { t: Date.now(), type:'delta', who, delta, reason });
}

function resetScores(){
  set(child("points"), { you:0, her:0 });
  push(child("history"), { t: Date.now(), type:'reset' });
}
...
function createGoal(){
  ...
  const idRef = push(child("goals"));
  const obj = { title, need, note, for: forWho, done:false, createdAt: Date.now() };
  set(idRef, obj);
  push(child("history"), { t: Date.now(), type:'goal-create', goal: obj.title, for: obj.for, need: obj.need });
}

function toggleGoal(id, current){
  update(child(`goals/${id}`), { done: !current });
  push(child("history"), { t: Date.now(), type:'goal-toggle', goalId:id, to:!current });
}

function deleteGoal(id, title){
  remove(child(`goals/${id}`));
  push(child("history"), { t: Date.now(), type:'goal-delete', goal:title });
}

function clearCompleted(){
  onValue(child("goals"), (snap)=>{
    const data = snap.val() || {};
    Object.entries(data).forEach(([id,g]) => { if (g.done) remove(child(`goals/${id}`)); });
  }, { onlyOnce:true });
}

function clearHistory(){ set(child("history"), null); }
function saveNames(you, her){ update(child("names"), { you, her }); }
...
function renderGoals(data){
  ...
  if (!items.length){
    list.innerHTML = `<div class="bg-white rounded-2xl shadow-soft p-6 text-slate-500">Пока нет целей — добавь первую.</div>`;
    return;
  }
